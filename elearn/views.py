...
def course(request):
    if request.method == 'POST':
        ...
        if name and color:
            if not Course.objects.filter(name=name, color=color).exists():
                a = Course(name=name, color=color)
                a.save()
                messages.success(request, 'New Course Was Registered Successfully')
                return redirect('course')
            else:
                messages.success(request, 'Course already taken!')
                return render(request, 'dashboard/admin/course.html')
        else:
            messages.success(request, 'Enter the valid inputs')
            return render(request, 'dashboard/admin/course.html')
    else:
        return render(request, 'dashboard/admin/course.html')
...
class ListAllTise(LoginRequiredMixin, ListView):
    model = Announcement
    template_name = 'dashboard/admin/update_announcements.html'
...
class QuizResultsView(DeleteView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # quiz_data =
        learners = [int(quiz.learner_id) for quiz in taken_quizzes]
        # print(learners)
        users = Profile.objects.filter(user_id__in=learners).values_list('user_id', 'user__username')
        # quiz_data = chain(users, taken_quizzes)
        # for data in quiz_data:
        #     print(data)
        # users = Profile.objects.filter(user_id__in=learners).values('user__username')
        print(users)
        quiz_data = list()

        print([users.values_list('user__username', flat=True)])
        total_taken_quizzes = taken_quizzes.count()
        quiz_score = quiz.taken_quizzes.aggregate(average_score=Avg('score'))
        for quiz in taken_quizzes:
            temp_dict = {'learner_id': users.filter(user_id=quiz.learner_id).values_list('user__username', flat=True)[0],
                         'date': quiz.date,
                         'score': quiz.score
                         }
            quiz_data.append(temp_dict)

        extra_context = {
            # 'taken_quizzes': taken_quizzes,
            'taken_quizzes': quiz_data,
...
class LTiseList(LoginRequiredMixin, ListView):
    model = Announcement
...
class TakenQuizListView(ListView):
    ...
    ##
    def get_context_data(self, **kwargs):
        taken_quizzes = self.request.user.learner.taken_quizzes.select_related('quiz', 'quiz__course').order_by('quiz__name')
        quizzes = [int(quiz.quiz_id) for quiz in taken_quizzes]
        quiz_names = Quiz.objects.filter(id__in=quizzes).values_list('id', 'name', 'course_id')
        ...
        for quiz in taken_quizzes:
            temp_dict = {'name': quiz_names.filter(id=quiz.quiz_id).values_list('name', flat=True)[0],
                         'sub_id': Course.objects.get(id=quiz_names.filter(id=quiz.quiz_id).values_list('course_id', flat=True)[0]),
                         'score': quiz.score
                         }
            quiz_data.append(temp_dict)
